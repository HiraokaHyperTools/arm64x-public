...
* ``x64`` も一部存在します。 但し、これは ``.hexpthk`` Hybrid Export Patch Thunk (何の略称であるかは諸説あり) という、 Prism エミュレーター内部で実行することを想定した、 16 バイト程度の JMP 用のコードの羅列です。 ビジネスロジックを記述するような性質のものでありません。
